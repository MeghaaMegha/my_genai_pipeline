...
def log_pipeline_experiment(run_name: str, parameters: dict, metrics: dict, tensor_artifacts: dict = None, vector_db_dir: str = None):
    """
    ...
    try:
        ...
        with mlflow.start_run(run_name=run_name):
            ...
            if tensor_artifacts:
                logger.info("Serializing processed BERT token tensors to local binary buffers.")
                
                # FIX: Dynamically build the path using current working directory
                temp_tensor_path = os.path.join(os.getcwd(), "bert_features.pkl")
                ...
                # Clean up local temporary file buffer safely
                if os.path.exists(temp_tensor_path):
                    os.remove(temp_tensor_path)
                logger.info("[SUCCESS] Serialized PyTorch tensor assets successfully archived in MLflow.")

            # 3. Snapshot and version the entire Chroma DB index directory
            if vector_db_dir and os.path.exists(vector_db_dir):
                logger.info(f"Archiving local persistent Vector Database folder: {vector_db_dir}")
                mlflow.log_artifacts(vector_db_dir, artifact_path="chroma_vector_db")
                logger.info("[SUCCESS] Chroma Vector DB snapshot successfully versioned in MLflow.")
            ...
            logger.info("[SUCCESS] Session run values and heavy file assets written into local registry layer.")
            
    except Exception as e:
        logger.error(f"Critical failure logging enterprise pipeline assets to MLflow: {e}")
        raise e
